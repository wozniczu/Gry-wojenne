...
{
    if (!assets::tryLoadUiFont(font)) {
        std::cerr << "Nie udało się załadować czcionki UI!" << std::endl;
    }
    if (!assets::tryLoadMenuTexture(menuTexture)) {
        std::cerr << "Nie udało się załadować tekstury menu (użyto zastępczego tła)." << std::endl;
    }
